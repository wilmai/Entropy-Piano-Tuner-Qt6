...
const qreal KeyboardGraphicsView::PEN_THIN_LINE =
    1 * KeyboardGraphicsView::GLOBAL_SCALING;
const qreal KeyboardGraphicsView::PEN_MEDIUM_LINE =
    2 * KeyboardGraphicsView::GLOBAL_SCALING;
const qreal KeyboardGraphicsView::KEY_WIDTH =
    10 * KeyboardGraphicsView::GLOBAL_SCALING;
const qreal KeyboardGraphicsView::KEY_HEIGHT =
    80 * KeyboardGraphicsView::GLOBAL_SCALING;
const qreal KeyboardGraphicsView::KEY_SPACING_TO_MARKERS =
    KeyboardGraphicsView::KEY_WIDTH + 2 * KeyboardGraphicsView::GLOBAL_SCALING;
const qreal KeyboardGraphicsView::KEY_MARKERS_MARGIN =
    1 * KeyboardGraphicsView::GLOBAL_SCALING;
const qreal KeyboardGraphicsView::TOTAL_HEIGHT =
    KeyboardGraphicsView::KEY_HEIGHT +
    KeyboardGraphicsView::KEY_SPACING_TO_MARKERS;
const qreal KeyboardGraphicsView::KEY_BLACK_TO_WHITE_RATIO = 0.6;
const qreal KeyboardGraphicsView::KEY_WHITE_KEY_SIZE =
    KeyboardGraphicsView::KEY_WIDTH * 12.0 / 7.0;
...
const QColor KeyboardGraphicsView::KEY_PRELIMINARY_AND_SELECTED_COLOR(255, 150,
                                                                      0);

KeyboardGraphicsView::KeyboardGraphicsView(QWidget *parent)
    : QGraphicsView(parent), mKeyboard(nullptr), mKeysGraphicsItems(0),
      mCenterOnKey(-1), mSelectedKey(-1),
      mSelectedKeyState(KeyState::STATE_NORMAL), mPreliminaryKey(-1),
      mKeyNumberOfA(-1) {
  GraphicsKeyItem::initShapes(KEY_WIDTH, KEY_HEIGHT * KEY_BLACK_TO_WHITE_RATIO,
                              KEY_WHITE_KEY_SIZE,
                              KEY_HEIGHT - 2 * PEN_THIN_LINE);

  setScene(&mScene);
  setWhatsThis(
      tr("This window displays the keyboard. Each key has a small indicator "
         "that will display the current recording state of this key."));
  ...
  probs.setScrollMetric(QScrollerProperties::VerticalOvershootPolicy,
...
void KeyboardGraphicsView::setKeyboard(const Keyboard *kb) {
  mKeyboard = kb;
  if (mKeyboard) {
    changeTotalNumberOfKeys(mKeyboard->getNumberOfKeys(),
                            mKeyboard->getKeyNumberOfA4());
  }
}

void KeyboardGraphicsView::setSelection(int key, KeyState state,
                                        int preliminary) {
...
void KeyboardGraphicsView::handleMessage(MessagePtr m) {
  ...
  case Message::MSG_KEY_SELECTION_CHANGED: {
    ...
    for (AutoScaledToKeyboardGraphicsView *scaledViews :
  ...
  case Message::MSG_PROJECT_FILE: {
    ...
    changeTotalNumberOfKeys(mpf->getPiano().getKeyboard().getNumberOfKeys(),
                            mpf->getPiano().getKeyboard().getKeyNumberOfA4());
    break;
  }
...
void KeyboardGraphicsView::setKeyColor(int8_t key) {
  ...
  QColor selectedKeyColor =
      getKeyColor(mKeyboard->getKeyColor(key), mSelectedKey == key,
                  mSelectedKeyState, mKeyboard->at(key).isRecorded());
  ...
  if (mPreliminaryKey == key && mSelectedKey == key) {
    ...
    mKeysGraphicsItems[mSelectedKey]->setBrush(
        QBrush(KEY_PRELIMINARY_AND_SELECTED_COLOR));
...
void KeyboardGraphicsView::selectKey(int8_t key0, piano::KeyState keyState,
  ...
  if (mSelectedKey >= 0) {
    ...
    if (mSelectedKey == mPreliminaryKey) {
      mKeysGraphicsItems[mPreliminaryKey]->setBrush(
          QBrush(KEY_PRELIMINARY_COLOR));
    }
  }
  ...
  if (notifyMessageListeners) {
    MessageHandler::send<MessageKeySelectionChanged>(
        key0, mKeyboard->getKeyPtr(key0), keyState);
...
QColor KeyboardGraphicsView::getKeyColor(piano::KeyColor color, bool selected,
                                         KeyState keyState, bool recorded) {
  if (selected) {
    if (recorded) {
...
QRectF KeyboardGraphicsView::keyShape(int8_t key0) {
  ...
  // white keys
  case 0:
    return QRectF((key0)*KEY_WIDTH, KEY_SPACING_TO_MARKERS, KEY_WIDTH * frac,
                  KEY_HEIGHT);
  case 2:
    return QRectF((key0 - 2 + 1 * frac) * KEY_WIDTH, KEY_SPACING_TO_MARKERS,
                  KEY_WIDTH * frac, KEY_HEIGHT);
  case 4:
    return QRectF((key0 - 4 + 2 * frac) * KEY_WIDTH, KEY_SPACING_TO_MARKERS,
                  KEY_WIDTH * frac, KEY_HEIGHT);
  case 5:
    return QRectF((key0 - 5 + 3 * frac) * KEY_WIDTH, KEY_SPACING_TO_MARKERS,
                  KEY_WIDTH * frac, KEY_HEIGHT);
  case 7:
    return QRectF((key0 - 7 + 4 * frac) * KEY_WIDTH, KEY_SPACING_TO_MARKERS,
                  KEY_WIDTH * frac, KEY_HEIGHT);
  case 9:
    return QRectF((key0 - 9 + 5 * frac) * KEY_WIDTH, KEY_SPACING_TO_MARKERS,
                  KEY_WIDTH * frac, KEY_HEIGHT);
  case 11:
    return QRectF((key0 - 11 + 6 * frac) * KEY_WIDTH, KEY_SPACING_TO_MARKERS,
                  KEY_WIDTH * frac, KEY_HEIGHT);

  // black keys
  default:
    return QRectF(key0 * KEY_WIDTH, KEY_SPACING_TO_MARKERS, KEY_WIDTH,
                  KEY_HEIGHT * KEY_BLACK_TO_WHITE_RATIO);
  }
}

void KeyboardGraphicsView::changeTotalNumberOfKeys(int keys, int keyA) {
  if (keys == static_cast<int>(mMarkerPixmapItems.size()) &&
      mKeyNumberOfA == keyA) {
  ...
  QGraphicsRectItem *shadowItem = mScene.addRect(
      0, 0, KEY_WIDTH * keys, KEY_HEIGHT, Qt::NoPen, QBrush(shadowGradient));
  ...
  for (int8_t i = 0; i < (int8_t)(mMarkerPixmapItems.size()); i++) {
    ...
    // marker pixmaps
    QGraphicsPixmapItem *pixmapItem = mScene.addPixmap(
        getPixmapForKeyRecordingState(mKeyboard->at(i).isRecorded()));
    ...
    GraphicsKeyItem *item =
        new GraphicsKeyItem(keyColorType, keyText, textColor);
    mScene.addItem(item);
    item->setPos(pos);
  ...
  setSceneRect(QRectF(0, 0, KeyboardGraphicsView::KEY_WIDTH * keys,
                      KeyboardGraphicsView::TOTAL_HEIGHT));

  for (auto *view : mAutoScaledGraphicsViews) {
    view->setSceneRect(QRectF(0, 0, KeyboardGraphicsView::KEY_WIDTH * keys,
                              KeyboardGraphicsView::TOTAL_HEIGHT));
  }
...
void KeyboardGraphicsView::selectNext() {
  setFocus();
  selectKey(std::min<int>(mSelectedKey + 1,
                          static_cast<int>(mKeysGraphicsItems.size()) - 1),
            STATE_NORMAL);
}
