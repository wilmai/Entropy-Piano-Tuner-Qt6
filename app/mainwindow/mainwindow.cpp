...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent),
      mIconPostfix((QApplication::primaryScreen()->devicePixelRatio() > 1.5)
                       ? "@2x"
                       : ""),
      mCore(nullptr), ui(new Ui::MainWindow) {
  ui->setupUi(this);
  ui->graphSplitter->setSizes(QList<int>() << 200 << 200);

  // this is in mm
  qreal maxDisplaySize =
      std::max(QGuiApplication::primaryScreen()->physicalSize().width(),
               QGuiApplication::primaryScreen()->physicalSize().height());
  new DisplaySizeDefines(
      maxDisplaySize * 0.0393701,
      ui->modeToolBar->fontMetrics().height()); // convert mm to inch
  ...
  QObject::connect(this, SIGNAL(modeChanged(OperationMode)),
                   mVolumeControlGroup, SLOT(onModeChanged(OperationMode)));
  QObject::connect(mVolumeControlGroup, SIGNAL(refreshInputLevels()), this,
                   SLOT(onResetNoiseLevel()));
  QObject::connect(mVolumeControlGroup, SIGNAL(muteMicroToggled(bool)), this,
                   SLOT(onToggleMicroMute(bool)));
  QObject::connect(mVolumeControlGroup, SIGNAL(muteSpeakerToggled(bool)), this,
                   SLOT(onToggleSpeakerMute(bool)));

  mSignalAnalyzerGroup = new SignalAnalyzerGroupBox(this);
  ui->controlLayout->addWidget(mSignalAnalyzerGroup, 0);
  QObject::connect(this, SIGNAL(modeChanged(OperationMode)),
                   mSignalAnalyzerGroup, SLOT(onModeChanged(OperationMode)));

  mTuningIndicatorGroup = new TuningIndicatorGroupBox(this);
  ui->controlLayout->addWidget(mTuningIndicatorGroup, 1);
  QObject::connect(this, SIGNAL(modeChanged(OperationMode)),
                   mTuningIndicatorGroup, SLOT(onModeChanged(OperationMode)));

  mKeyboardGraphicsView = new KeyboardGraphicsView(this);
  qobject_cast<QBoxLayout *>(ui->centralWidget->layout())
      ->addWidget(mKeyboardGraphicsView);
  ...
  QScroller::grabGesture(modeScrollArea->viewport(),
                         QScroller::LeftMouseButtonGesture);
  QWidget *modeScrollContents = new QWidget;
  modeScrollArea->setWidget(modeScrollContents);
  modeScrollContents->setSizePolicy(QSizePolicy::Preferred,
  ...
  recordingButton->setIcon(
      QIcon::fromTheme("audio-input-microphone",
                       QIcon(":/media/icons/audio-input-microphone.png")));
  recordingButton->setWhatsThis(
      tr("Press this button to switch to the recording mode."));
  connect(recordingButton, SIGNAL(toggled(bool)), this,
          SLOT(onModeRecord(bool)));
  ...
  calculatingButton->setWhatsThis(
      tr("Press this button to switch to the calculation mode."));
  connect(calculatingButton, SIGNAL(toggled(bool)), this,
          SLOT(onModeCalculate(bool)));
  ...
  tuningButton->setWhatsThis(
      tr("Press this button to switch to the tuning mode."));
  ...
  auto iconFromTheme = [](const QString &s) {
    return QIcon::fromTheme(s, QIcon(":/media/icons/" + s + ".png"));
  };
  ...
  mFileToolBar->addAction(iconFromTheme("document-new"), tr("New"), this,
                          SLOT(onFileNew()));
  mFileToolBar->addAction(iconFromTheme("document-open"), tr("Open"), this,
                          SLOT(onFileOpen()));
  mFileToolBar->addAction(iconFromTheme("document-save"), tr("Save"), this,
                          SLOT(onFileSave()));
  mFileToolBar->addAction(iconFromTheme("document-save-as"), tr("Save as"),
                          this, SLOT(onFileSaveAs()));
  mFileToolBar->addAction(iconFromTheme("edit-piano-sheet"),
                          tr("Edit piano data sheet"), this,
                          SLOT(onEditPianoDataSheet()));
  mFileToolBar->addAction(iconFromTheme("edit-clear"),
                          tr("Clear pitch markers"), this,
                          SLOT(onResetRecording()));
  mFileToolBar->addSeparator();
  mFileToolBar->addAction(iconFromTheme("preferences-system"), tr("Options"),
                          this, SLOT(onOptions()));
  mFileToolBar->addAction(QIcon(":/media/icons/mathematical_plot.png"),
                          tr("Graphs"), this, SLOT(onOpenPlots()));
  ...
  helpToolBar->addAction(iconFromTheme("help-contents"), tr("Tutorial"), this,
                         SLOT(onTutorial()));
  // helpToolBar->addAction(iconFromTheme("accessories-text-editor"), tr("Log"),
  // this, SLOT(onViewLog()));
  helpToolBar->addAction(iconFromTheme("help-about"), tr("About"), this,
                         SLOT(onAbout()));
  ...
  QObject::connect(exportAction, SIGNAL(triggered(bool)), this,
                   SLOT(onExport()));
  ...
  QBoxLayout *mainLayout =
      qobject_cast<QBoxLayout *>(centralWidget()->layout());
...
void MainWindow::init(Core *core) {
  ...
  connect(mMidiAutoConnector, &QMidiAutoConnector::inputDeviceCreated, this,
          &MainWindow::onMidiInputDeviceCreated);
  ...
  // hide some elements
  if (DisplaySizeDefines::getSingleton()->getGraphDisplayMode() ==
      GDM_ONE_VISIBLE) {
    ui->fourierSpectrumGraphics->setVisible(
        false); // only visible during recording

    // hide status bar
    statusBar()->setHidden(true);
  }

  ui->controlLayout->addWidget(mCalculationProgressGroup =
                                   new CalculationProgressGroup(mCore, this));
...
void MainWindow::start() {
  ...
  if (mCore->getSoundGenerator()) {
    mCore->getSoundGenerator()
        ->getSynthesizer()
        .getWaveformGenerator()
        .addListener(mProgressDisplay);
  }
  ...
  QObject::connect(versionChecker, SIGNAL(updateAvailable(VersionInformation)),
                   this, SLOT(onVersionUpdate(VersionInformation)));
}
...
void MainWindow::handleMessage(MessagePtr m) {
  ...
  case Message::MSG_MODE_CHANGED: {
    ...
    if (mCore && mCore->getProjectManager()->hasChangesInFile()) {
      if (DoNotShowAgainMessageBox::show(
              DoNotShowAgainMessageBox::MODE_CHANGE_SAVE, this) ==
          QMessageBox::Yes) {
        // save
        if (mCore->getProjectManager()->onSaveFile() ==
            ProjectManagerAdapter::R_CANCELED) {
          // if the user cancels the dialog, reset the save state of the dialog
          SettingsForQt::getSingleton().setDoNotShowAgainMessageBox(
              DoNotShowAgainMessageBox::MODE_CHANGE_SAVE, false, -1);
        }
      }
    }
    ...
    if (mmc->getMode() == MODE_TUNING) {
      ...
      for (const Key &key :
           mCore->getPianoManager()->getPiano().getKeyboard().getKeys()) {
        if (!key.isRecorded()) {
      ...
      if (missingKeyRecordings > 0 &&
          !DoNotShowAgainMessageBox::doNotShowAgain(
              DoNotShowAgainMessageBox::NOT_ALL_KEYS_RECORDED)) {
        // manually check if reshow, that we reach other else and if's
        // we do not want to show more than one box at a time
        DoNotShowAgainMessageBox::show(
            DoNotShowAgainMessageBox::NOT_ALL_KEYS_RECORDED, this);
      } else if (missingComputedFrequencies == mCore->getPianoManager()
                                                   ->getPiano()
                                                   .getKeyboard()
                                                   .getNumberOfKeys()) {
        DoNotShowAgainMessageBox::show(
            DoNotShowAgainMessageBox::TUNING_CURVE_NOT_CALCULATED, this);
      } else if (missingComputedFrequencies > 0) {
        DoNotShowAgainMessageBox::show(
            DoNotShowAgainMessageBox::TUNING_CURVE_NEEDS_UPDATE, this);
      }
    } else {
    }
    ...
    if (msa->status() == MessageSignalAnalysis::Status::STARTED) {
      ...
      case MessageSignalAnalysis::Result::INVALID:
        ...
        if (!mClosing && mCurrentMode == MODE_RECORDING) {
          if (msa->invalidAttempts() >=
              MAX_NUMBER_OF_INVALID_RECORDINGS_BEFORE_USER_INFORMATION) {
            LogD("Maximum number of invalid recordings reached. Trying to "
                 "inform the user.");
            DoNotShowAgainMessageBox::show(
                DoNotShowAgainMessageBox::FORCE_RECORDING_INFORMATION, this);
          }
        }
      }
    }
...
void MainWindow::changesInFileUpdated(bool) { updateWindowTitle(); }

void MainWindow::updateNoteName(int key) {
  QString text(QString::fromStdString(
      mCore->getPianoManager()->getPiano().getKeyboard().getNoteName(key)));
  // add super script to #
...
void MainWindow::updateWindowTitle() {
  QFileInfo fi(QString::fromStdWString(
      mCore->getProjectManager()->getCurrentFilePath()));
  QString asterix = mCore->getProjectManager()->hasChangesInFile() ? "*" : "";
  QString pianoName(
      QString::fromStdWString(mCore->getPianoManager()->getPiano().getName()));
...
void MainWindow::updateVolumeBar() {
  mVolumeControlGroup->updateLevels(mCore->getAudioRecorder()->getStopLevel(),
                                    AudioRecorder::LEVEL_TRIGGER);
}
...
void MainWindow::onOpenSoundControl() {
  ...
  programNames.append(
      qMakePair(QString("gnome-control-center"), QStringList() << "sound"));
#elif __APPLE__
  // not supported yet
  programNames.append(qMakePair(
      QString("open"),
      QStringList() << "/System/Library/PreferencePanes/Sound.prefPane/"));
#elif _WIN32
  // untested, see http://support.microsoft.com/kb/192806/de
  programNames.append(
      qMakePair(QString("control"), QStringList() << "mmsys.cpl" << "sounds"));
#endif
  ...
  for (auto &program : programNames) {
    ...
    if (process->waitForStarted()) {
      LogI("Sound control program '%s' started",
           program.first.toStdString().c_str());
      return;
    }
  }
  ...
  QMessageBox::warning(this, tr("Warning"),
                       tr("Unable to find a supported sound control."));
#endif
}
...
void MainWindow::onFileNew() { mCore->getProjectManager()->onNewFile(); }

void MainWindow::onFileSave() { mCore->getProjectManager()->onSaveFile(); }

void MainWindow::onFileSaveAs() { mCore->getProjectManager()->onSaveFileAs(); }

void MainWindow::onFileOpen() { mCore->getProjectManager()->onOpenFile(); }

void MainWindow::onFileShare() {
  if (mCore->getProjectManager()->onShare() !=
      ProjectManagerAdapter::R_ACCEPTED) {
    QMessageBox::information(
        this, tr("Canceled"),
        tr("You need to save the file before you can share it."));
  }
}
...
void MainWindow::onResetRecording() {
  if (DoNotShowAgainMessageBox::show(DoNotShowAgainMessageBox::RESET_PITCHES,
...
void MainWindow::onTutorial() {
  ...
  helpDialog.setGeometry(mwRect.x() + mwRect.width() / 4,
                         mwRect.y() + mwRect.height() / 4, mwRect.width() / 2,
                         mwRect.height() / 2);
  ...
  text.replace(QRegularExpression("SRC=\"([^\"]*)\""),
               "SRC=\":/tutorial/\\1\"");
  edit->setText(text);
  edit->setTextInteractionFlags(Qt::LinksAccessibleByMouse |
...
void MainWindow::onManual() {
  QDesktopServices::openUrl(
      QUrl(serverinfo::getManualFileAddress(
               SettingsForQt::getSingleton().getUserLanguageId())
               .c_str()));
}
...
void MainWindow::onSelectNextMode() {
  OperationMode mode =
      static_cast<OperationMode>((mCurrentMode + 1) % MODE_COUNT);
  mModeToolButtons[mode]->setChecked(true);
}

void MainWindow::onSelectPreviousMode() {
  OperationMode mode =
      static_cast<OperationMode>((mCurrentMode - 1 + MODE_COUNT) % MODE_COUNT);
  mModeToolButtons[mode]->setChecked(true);
}
...
void MainWindow::onToggleTuningIndictator() {
  if (mTuningIndicatorGroup)
    mTuningIndicatorGroup->getTunincIndicatorView()
        ->toggleSpectralAndStroboscopeMode();
}

void MainWindow::onExport() { mCore->getProjectManager()->onExport(); }

void MainWindow::onVersionUpdate(VersionInformation information) {
  QMessageBox mb;
  mb.setText(tr("A new update is available!"));
  mb.setInformativeText(
      tr("The online app version is %1. Do you want to install this update?")
          .arg(information.mAppVersion));
  mb.setStandardButtons(QMessageBox::Yes | QMessageBox::No);
  if (mb.exec() == QMessageBox::Yes) {
#ifdef Q_OS_MACX
    // download new dmg file
    if (!QDesktopServices::openUrl(QUrl::fromUserInput(
            serverinfo::getDownloadsFileAddress(serverinfo::MAC_X86_64_DMG)
                .c_str()))) {
      LogW("Online installer file for mac could not be found.");
      QDesktopServices::openUrl(
          QUrl::fromUserInput(serverinfo::SERVER_ADDRESS.c_str()));
    } else {
      this->close();
    }
#else
#ifdef QT_NO_PROCESS
    // no processes allowed on this device, usually mobile devices (e.g.
    // WinRT/WinPhone)
#else
    // run maintenace tool in updater mode
    if (QProcess::startDetached("maintenancetool",
                                QStringList() << "--updater") == false) {
      LogW("Maintenace tool could not be started.");
      QMessageBox::warning(this, tr("Warning"),
                           tr("The maintenance tool could not be started "
                              "automatically. To update the program you have "
                              "to start the maintenance tool manually."));
    } else {
...
void MainWindow::onMidiMessageReceived(const QMidiMessage &message) {
  mCore->getMidiInterface()->receiveMessage(
      message.byte0(), message.byte1(), message.byte2(), message.timestamp());
}
