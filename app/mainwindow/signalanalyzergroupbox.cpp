...
SignalAnalyzerGroupBox::SignalAnalyzerGroupBox(QWidget *parent)
    : DisplaySizeDependingGroupBox(parent, new QHBoxLayout,
  ...
  QHBoxLayout *mainLayout =
      qobject_cast<QHBoxLayout *>(mMainWidgetContainer->layout());
  mainLayout->setSpacing(0);

  RecordingQualityBar *qualityBar = new RecordingQualityBar(this);
  mainLayout->addWidget(qualityBar);

  RecordingStatusGraphicsView *statusView =
      new RecordingStatusGraphicsView(this);
  ...
  mFrequencyLabel->setFixedWidth(
      mFrequencyLabel->fontMetrics().horizontalAdvance("0000.0"));
  ...
  mFrequencyLabel->setWhatsThis(
      tr("This label shows the ground frequency of the selected key."));
