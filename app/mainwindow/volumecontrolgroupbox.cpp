...
VolumeControlGroupBox::VolumeControlGroupBox(QWidget *parent)
    : DisplaySizeDependingGroupBox(parent, new QVBoxLayout,
                                   toFlag(MODE_IDLE) | toFlag(MODE_RECORDING) |
                                       ((DisplaySizeDefines::getSingleton()
                                             ->showVolumeGroupBoxInTuningMode())
                                            ? toFlag(MODE_TUNING)
                                            : 0)) {
  ...
  QVBoxLayout *mainLayout =
      qobject_cast<QVBoxLayout *>(mMainWidgetContainer->layout());
  ...
  refreshButton->setIconSize(
      QSize(1, 1) * DisplaySizeDefines::getSingleton()->getMediumIconSize());
  QObject::connect(refreshButton, SIGNAL(clicked(bool)), this,
  ...
  muteMicroIcons.addFile(QStringLiteral(":/media/images/micro_active.png"),
                         QSize(), QIcon::Normal, QIcon::Off);
  muteMicroIcons.addFile(QStringLiteral(":/media/images/micro_mute.png"),
                         QSize(), QIcon::Normal, QIcon::On);
  muteMicroButton->setIcon(muteMicroIcons);
  muteMicroButton->setIconSize(
      QSize(1, 1) * DisplaySizeDefines::getSingleton()->getMediumIconSize());
  QObject::connect(muteMicroButton, SIGNAL(toggled(bool)), this,
                   SLOT(onMicroMuteToggled(bool)));
  ...
  if (DisplaySizeDefines::getSingleton()->showMuteOutputButton()) {
    ...
    muteSpeakerIcons.addFile(
        QStringLiteral(":/media/images/speaker_active.png"), QSize(),
        QIcon::Normal, QIcon::Off);
    muteSpeakerIcons.addFile(QStringLiteral(":/media/images/speaker_mute.png"),
                             QSize(), QIcon::Normal, QIcon::On);
    muteSpeakerButton->setIcon(muteSpeakerIcons);
    muteSpeakerButton->setIconSize(
        QSize(1, 1) * DisplaySizeDefines::getSingleton()->getMediumIconSize());
    QObject::connect(muteSpeakerButton, SIGNAL(toggled(bool)), this,
                     SLOT(onSpeakerMuteToggled(bool)));
    new QShortcut(QKeySequence(Qt::Key_S), muteSpeakerButton, SLOT(toggle()));
    muteSpeakerButton->setWhatsThis(
        tr("Click this button to mute the speaker or headphone."));
  }
  ...
  // set what's this textsClick this button to reset the automatic calibration
  // of the input volume.
  this->setWhatsThis(tr("This widgets provides settings and information about "
                        "the input level of the input device."));
  offLabel->setWhatsThis(
      tr("If the input level drops below this mark the recorder stops and does "
         "not process the input signal."));
  onLabel->setWhatsThis(
      tr("If the input level reaches this threshold the recorder starts "
         "analyzing the signal of the input device until the level drops below "
         "the 'Off' mark."));
  refreshButton->setWhatsThis(tr("Click this button to reset the automatic "
                                 "calibration of the input volume."));
  muteMicroButton->setWhatsThis(
      tr("Click this button to mute the input device."));
...
void VolumeControlGroupBox::onRefreshClicked() { emit refreshInputLevels(); }
