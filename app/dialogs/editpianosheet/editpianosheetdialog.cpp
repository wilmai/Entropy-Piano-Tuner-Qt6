...
EditPianoSheetDialog::EditPianoSheetDialog(const Piano &piano, QWidget *parent)
    : QDialog(parent, Qt::Window), ui(new Ui::EditPianoSheetDialog) {
  ui->setupUi(this);
  setModal(true);

  // if (DisplaySizeDefines::getSingleton()->isLEq(DS_XSMALL)) {
  //    // on small devices we need a bit more space for translated languages
  //    (e.g. German), reduce margins in general for xsmall devices
  //    layout()->setContentsMargins(3, 3, 3, 3);
  //    ui->scrollAreaWidgetContents->layout()->setContentsMargins(1, 1, 1, 1);
  // }
  ...
  ui->serialNumberLineEdit->setText(
      QString::fromStdWString(piano.getSerialNumber()));
  // we only need the year
  ui->yearEdit->setDate(
      QDate(QString::fromStdWString(piano.getManufactionYear()).toInt(), 1, 1));
  ui->productionLocationLineEdit->setText(
      QString::fromStdWString(piano.getManufactionLocation()));

  ui->tuningLocationLineEdit->setText(
      QString::fromStdWString(piano.getTuningLocation()));
  QDateTime time =
      QDateTime::fromString(QString::fromStdWString(piano.getTuningTime()),
                            "yyyy-MM-dd HH:mm:ss")
          .toLocalTime();
  ...
  ui->keyNumberOfASpinBox->setValue(piano.getKeyboard().getKeyNumberOfA4() +
                                    1); // counting starts at 1 (not at 0)
  ui->keysOnBassBridgeSpinBox->setValue(
      piano.getKeyboard().getNumberOfBassKeys());
  ...
  if (DisplaySizeDefines::getSingleton()->showMultiLineEditPianoDataSheet()) {
    ui->pianoManufacturerInformationLayout->setRowWrapPolicy(
        QFormLayout::WrapAllRows);
    ui->pianoOperatingSiteInformationLayout->setRowWrapPolicy(
        QFormLayout::WrapAllRows);
  } else {
    ui->pianoManufacturerInformationLayout->setRowWrapPolicy(
...
EditPianoSheetDialog::~EditPianoSheetDialog() { delete ui; }
...
void EditPianoSheetDialog::applyData(Piano *piano) const {
  ...
  // we only need the year
  piano->setManufactureYear(
      QString("%1").arg(ui->yearEdit->date().year()).toStdWString());
  piano->setManufactureLocation(
      ui->productionLocationLineEdit->text().toStdWString());

  piano->setTuningLocation(ui->tuningLocationLineEdit->text().toStdWString());
  piano->setTuningTime(ui->timeOfTuningDateTimeEdit->dateTime()
                           .toUTC()
                           .toString("yyyy-MM-dd HH:mm:ss")
                           .toStdWString());
  piano->setConcertPitch(ui->concertPitchSpinBox->value());
  piano->getKeyboard().setNumberOfBassKeys(
      ui->keysOnBassBridgeSpinBox->value());

  piano->getKeyboard().changeKeyboardConfiguration(
      ui->numberOfKeysSpinBox->value(),
