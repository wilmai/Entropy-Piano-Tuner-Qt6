...
AboutDialog::AboutDialog(QWidget *parent, QString iconPostfix)
  ...
  setGeometry(r.left() + r.width() / 4, r.top() + r.height() / 4, r.width() / 2,
              r.height() / 2);
  ...
  titleLayout->addWidget(
      new QLabel(QString("<h1>%1 %2</h1>")
                     .arg(tr("Entropy Piano Tuner"), EPT_VERSION_STRING)));
  titleLayout->addStretch();

  QLabel *webpage = new QLabel(QString::fromStdString(
      "<a href=\"" + serverinfo::SERVER_ADDRESS + "\">" +
      serverinfo::SERVER_NAME + "</a> - <a href=\"mailto:.org\">Feedback</a>"));
  QObject::connect(webpage, SIGNAL(linkActivated(QString)), this,
                   SLOT(onOpenAboutLink(QString)));
  ...
  QObject::connect(text, SIGNAL(anchorClicked(QUrl)), this,
                   SLOT(onOpenAboutUrl(QUrl)));
  ...
  const QString buildText =
      tr("Built on %1")
          .arg(QLocale().toString(compileTime, QLocale::LongFormat));
  const QString buildByText =
      tr("by %1 and %2")
          .arg("Prof. Dr. Haye Hinrichsen", "Christoph Wick M.Sc.");

  QString dependenciesText = tr("Based on");
  dependenciesText.append(" <a href=\"Qt\">Qt</a>, <a "
                          "href=\"https://gitlab.com/tp3/qtmidi\">QtMidi</a>, "
                          "<a href=\"http://qwt.sf.net\">Qwt</a>, <a "
                          "href=\"http://fftw.org\">fftw3</a>");
  ...
  dependenciesText.append(
      ", <a "
      "href=\"https://github.com/kshoji/USB-MIDI-Driver\">USB-Midi-Driver</a>");
#else
  dependenciesText.append(
  ...
  const QString copyrightText =
      tr("Copyright %1 Dept. of Theor. Phys. III, University of Würzburg. All "
         "rights reserved.")
          .arg(compileTime.toString("yyyy"));
  const QString licenseText =
      tr("This software is licensed under the terms of the %1. The source code "
         "can be accessed at %2.")
          .arg(
              "<a "
              "href=\"http://www.gnu.org/licenses/"
              "gpl-3.0-standalone.html\">GPLv3</a>",
              "<a "
              "href=\"https://gitlab.com/tp3/Entropy-Piano-Tuner\">GitLab</a>");

  const QString warrantyText =
      tr("The program is provided AS IS with NO WARRANTY OF ANY KIND, "
         "INCLUDING THE WARRANTY OF DESIGN, MERCHANTABILITY AND FITNESS FOR A "
         "PARTICULAR PURPOSE.");

  const QString acknowledgementsText =
      tr("We thank all those who have contributed to the project:") +
      " Prof. Dr. S. R. Dahmen, A. Frick, A. Heilrath, M. Jiminez, Prof. Dr. "
      "W. Kinzel, M. Kohl, L. Kusmierz, Prof. Dr. A. C. Lehmann, B. Olbrich., "
      "Dr. J. Um, Zhou Ying, Xavier Monnin";

  auto makeParagraphTags = [](const QString &t) { return "<p>" + t + "</p>"; };
  QString completeText;
  completeText.append("<html><img src=\"" + iconPath +
                      "\" style=\"float: left;\"/>");
