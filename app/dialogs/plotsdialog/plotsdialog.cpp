...
PlotsDialog::PlotsDialog(const Piano &piano, QWidget *parent)
    : QDialog(parent, Qt::Window), mPiano(piano),
      mKeyboard(piano.getKeyboard()) {
  std::fill(mCurves.begin(), mCurves.end(), nullptr);
  const bool vNav(
      DisplaySizeDefines::getSingleton()->showPlotNavigationVertical());

  CentralPlotFrame *plot =
      new CentralPlotFrame(mKeyboard.getNumberOfKeys(), getKeyOffset());
  ...
  if (vNav) {
    ...
    vTbExtraLayout->setContentsMargins(0, 0, 0, 0);
  ...
  auto makeToolButton = [&toolBar, &makeColorIcon,
                         this](QColor c, QString title, Curves curve) {
    ...
    QObject::connect(b, SIGNAL(toggled(bool)), this,
                     SLOT(plotToolButtonToggled(bool)));
  ...
  navBar->addAction(QIcon(":/media/icons/view-fullscreen.png"),
                    tr("Reset view"), plot, SLOT(resetView()));
  navBar->addAction(QIcon(":/media/icons/go-first.png"), tr("Go first"), plot,
                    SLOT(zoomGoFirst()));
  navBar->addAction(QIcon(":/media/icons/go-previous.png"), tr("Go previous"),
                    plot, SLOT(zoomGoPrevious()));
  navBar->addAction(QIcon(":/media/icons/go-next.png"), tr("Go next"), plot,
                    SLOT(zoomGoNext()));
  navBar->addAction(QIcon(":/media/icons/go-last.png"), tr("Go last"), plot,
                    SLOT(zoomGoLast()));
  ...
  QObject::connect(mPlot, SIGNAL(keyWidthChanged(double)), this,
                   SLOT(updateTickWidth(double)));

  // show last shown charts
  QSettings s;
  for (int i = 0; i < CURVE_COUNT; i++) {
    bool defaultShown = i == CURVE_RECORDED || i == CURVE_COMPUTED;
    mPlotToolButtons[i]->setChecked(
        s.value(QString("plots/curve%1").arg(i), defaultShown).toBool());
  }
  ...
  setWindowState(
      static_cast<Qt::WindowState>(s.value("plots/windowState").toInt()));
#else
  SHOW_DIALOG(this);
#endif
...
int PlotsDialog::getKeyOffset() { return 48 - mKeyboard.getKeyNumberOfA4(); }

void PlotsDialog::prepareCurve(Curves curve) {
  if (mCurves[curve]) {
    delete mCurves[curve];
  }
  ...
  if (curve == CURVE_INHARMONICITY) {
    ...
    c->setSymbol(new QwtSymbol(QwtSymbol::HLine, QBrush(),
                               QPen(Qt::darkGreen, 2),
                               QSize(mPlot->currentTickDistanceInPixel(), 1)));

    QPolygonF points;
    for (int i = 0; i < mKeyboard.getNumberOfKeys(); ++i) {
      if (mKeyboard[i].getMeasuredInharmonicity() > 0) {
        points << QPointF(i + 0.5 + getKeyOffset(),
                          mKeyboard[i].getMeasuredInharmonicity());
      }
    }
    c->setSamples(points);
  } else if (curve == CURVE_RECORDED) {
    c->setStyle(QwtPlotCurve::NoCurve);
    c->setSymbol(new QwtSymbol(QwtSymbol::HLine, QBrush(), QPen(Qt::red, 2),
                               QSize(mPlot->currentTickDistanceInPixel(), 1)));
    ...
    for (int i = 0; i < mKeyboard.getNumberOfKeys(); ++i) {
      points << QPointF(i + 0.5 + getKeyOffset(),
                        cents(mKeyboard[i].getRecordedFrequency() /
                              mPiano.getEqualTempFrequency(i, 0, fA4)));
    }
    c->setSamples(points);
  } else if (curve == CURVE_COMPUTED) {
    c->setStyle(QwtPlotCurve::NoCurve);
    c->setSymbol(new QwtSymbol(QwtSymbol::HLine, QBrush(), QPen(Qt::blue, 2),
                               QSize(mPlot->currentTickDistanceInPixel(), 1)));

    QPolygonF points;
    for (int i = 0; i < mKeyboard.getNumberOfKeys(); ++i) {
      points << QPointF(i + 0.5 + getKeyOffset(),
                        cents(mKeyboard[i].getComputedFrequency() /
                              mPiano.getDefiningTempFrequency(i, 0, 440)));
    }
    c->setSamples(points);
  } else if (curve == CURVE_TUNED) {
    c->setStyle(QwtPlotCurve::NoCurve);
    c->setSymbol(new QwtSymbol(QwtSymbol::HLine, QBrush(), QPen(Qt::green, 2),
                               QSize(mPlot->currentTickDistanceInPixel(), 1)));

    QPolygonF points;
    for (int i = 0; i < mKeyboard.getNumberOfKeys(); ++i) {
      points << QPointF(i + 0.5 + getKeyOffset(),
                        cents(mKeyboard[i].getTunedFrequency() /
                              mPiano.getEqualTempFrequency(i)));
    }
    c->setSamples(points);
  }
