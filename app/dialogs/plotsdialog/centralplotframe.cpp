...
CentralPlotFrame::CentralPlotFrame(int numberOfKeys, int keyOffset)
    : mNumberOfKeys(numberOfKeys), mKeyOffset(keyOffset) {
  // important: accept touch events, so they dont get propagated
  setAttribute(Qt::WA_AcceptTouchEvents);

  setAxisScaleEngine(xBottom,
                     new KeyIndexScaleEngine(numberOfKeys, mKeyOffset));

  KeyIndexScaleDraw *xScaleDraw = new KeyIndexScaleDraw;
  setAxisScaleDraw(xBottom, xScaleDraw);
  QObject::connect(this, SIGNAL(keyWidthChanged(double)), xScaleDraw,
                   SLOT(setKeyWidth(double)));

  // panning with the left mouse button
  QwtPlotPanner *panner = new QwtPlotPanner(canvas());
  panner->setMouseButton(Qt::LeftButton, Qt::ControlModifier);
  QObject::connect(this, SIGNAL(moveCanvas(int, int)), panner,
...
void CentralPlotFrame::applyTouchTransform(int final) {
  bool isLogX =
      dynamic_cast<QwtLogScaleEngine *>(axisScaleEngine(xBottom)) != nullptr;
  bool isLogY =
      dynamic_cast<QwtLogScaleEngine *>(axisScaleEngine(yLeft)) != nullptr;
  ...
  const QList<QEventPoint> &points(mTouchPoints);
  if (points.size() == 1) {
    const QEventPoint &p(points.first());
    const QPointF pPrev(invTransform(p.lastPosition()));
    const QPointF pCurr(invTransform(p.position()));
    ...
    const QEventPoint &p1(points.first());
    const QEventPoint &p2(points.last());

    const QPointF p1Last = invTransform(p1.lastPosition());
    const QPointF p2Last = invTransform(p2.lastPosition());
    const QPointF p1Curr = invTransform(p1.position());
    const QPointF p2Curr = invTransform(p2.position());

    // auto linearTransform = [](qreal x, qreal m, qreal t) {return m * x + t;};
    auto invLinearTransform = [](qreal x, qreal m, qreal t) {
      return (x - t) / m;
    };

    // auto logTransform = [](qreal x, qreal m, qreal t) {return std::pow(x, m)
    // * t;};
    auto invLogTransform = [](qreal x, qreal m, qreal t) {
      return std::pow(x / t, 1 / m);
    };

    auto linM = [](qreal p1, qreal c1, qreal p2, qreal c2) {
      return (c1 - c2) / (p1 - p2);
    };
    auto logM = [](qreal p1, qreal c1, qreal p2, qreal c2) {
      return log(c1 / c2) / log(p1 / p2);
    };

    auto linT = [](qreal p1, qreal c1, qreal m) { return c1 - m * p1; };
    auto logT = [](qreal p1, qreal c1, qreal m) {
...
bool CentralPlotFrame::touchEvent(QTouchEvent *e) {
  QList<QEventPoint> points(e->points());
  for (auto it = points.begin(); it != points.end();) {
    if (it->state() == QEventPoint::Released ||
        it->state() == QEventPoint::Pressed) {
      it = points.erase(it);
    } else {
      it++;
    }
...
void CentralPlotFrame::zoomGoNext() { mPlotZoomer->zoom(1); }

void CentralPlotFrame::zoomGoPrevious() { mPlotZoomer->zoom(-1); }
