...
AlgorithmDialog::AlgorithmDialog(
    std::shared_ptr<const AlgorithmInformation> currentAlgorithm, Piano &piano,
    QWidget *parent)
    : QDialog(parent), mPiano(piano) {
  EptAssert(currentAlgorithm, "Current algorithm has to exist.");

  QRect pr(parent->contentsRect());
  setGeometry(pr.center().x() - pr.width() / 4,
              pr.center().y() - pr.height() / 4, pr.width() / 2,
              pr.height() / 2);
  ...
  if (mAlgorithmNames.size() == 0) {
    for (auto &desc : CalculationManager::getSingleton().getAlgorithms()) {
      ...
      try {
        auto info(
            CalculationManager::getSingleton().loadAlgorithmInformation(name));
        mAlgorithmNames.push_back(
            qMakePair(QString::fromStdString(name),
                      QString::fromStdWString(info->getName())));
      } catch (...) {
        LogW("Error during loading and adding the algorithm '%s'. Skipping...",
             name.c_str());
      }
    }
  }
  ...
  comboBox->setCurrentIndex(
      comboBox->findData(QString::fromStdString(currentAlgorithm->getId())));
  EptAssert(comboBox->currentIndex() >= 0,
            "The default algotihm doesnt exist!");

  QObject::connect(comboBox, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(algorithmSelectionChanged(int)));
  ...
  mAlgorithmDescriptionScrollArea->setHorizontalScrollBarPolicy(
      Qt::ScrollBarAlwaysOff);

  QScroller::grabGesture(mAlgorithmDescriptionScrollArea->viewport(),
...
bool AlgorithmDialog::eventFilter(QObject *o, QEvent *e) {
  if (o == mAlgorithmDescriptionScrollArea->widget() &&
      e->type() == QEvent::Resize) {
    QWidget *contents = mAlgorithmDescriptionScrollArea->widget();
    QScrollBar *vScrollBar =
        mAlgorithmDescriptionScrollArea->verticalScrollBar();

    setMinimumWidth(contents->minimumSizeHint().width() + vScrollBar->width() +
...
void AlgorithmDialog::acceptCurrent() {
  ...
  for (auto &paramWidget : mAlgorithmWidgetConnectionList) {
    const AlgorithmParameterDescription &param =
        mCurrentAlgorithmInformation->getParameter(paramWidget.first);
    ...
    if (param.getType() == AlgorithmParameterDescription::TYPE_DOUBLE) {
      if (param.displaySpinBox()) {
        ...
        mCurrentAlgorithmParameters->setDoubleParameter(paramWidget.first,
                                                        sb->value());
        ...
        EPT_EXCEPT(EptException::ERR_INVALIDPARAMS,
                   "Either display spin box or line edit have to be set.");
      }
      ...
      if (param.displaySpinBox()) {
        ...
        mCurrentAlgorithmParameters->setIntParameter(paramWidget.first,
                                                     sb->value());
      } else if (param.displayLineEdit()) {
        const QLineEdit *le = dynamic_cast<const QLineEdit *>(widget);
        EptAssert(le, "This parameter is described by a QLineEdit");
      ...
      mCurrentAlgorithmParameters->setStringParameter(
          paramWidget.first, cb->currentData().toString().toStdString());
    } else if (param.getType() == AlgorithmParameterDescription::TYPE_BOOL) {
      const QCheckBox *cb = qobject_cast<const QCheckBox *>(widget);
      mCurrentAlgorithmParameters->setBoolParameter(paramWidget.first,
                                                    cb->isChecked());
    } else {
      EPT_EXCEPT(EptException::ERR_NOT_IMPLEMENTED,
...
void AlgorithmDialog::algorithmSelectionChanged(int index) {
  ...
  acceptCurrent();
  if (mAlgorithmDescriptionScrollArea->widget()) {
    delete mAlgorithmDescriptionScrollArea->widget();
  }
  ...
  mCurrentAlgorithmParameters =
      mPiano.getAlgorithmParameters().getOrCreate(algId.toStdString());
  mCurrentAlgorithmInformation =
      CalculationManager::getSingleton().loadAlgorithmInformation(
          mCurrentAlgorithmParameters->getAlgorithmName());
  ...
  setWindowTitle(
      tr("Info of algorithm: %1").arg(QString::fromStdWString(info.getName())));

  QWidget *scrollContentWidget = new QWidget;
  scrollContentWidget->setSizePolicy(QSizePolicy::Preferred,
  ...
  layout->addRow(new QLabel(tr("Name:")),
                 new QLabel(QString::fromStdWString(info.getName())));
  layout->addRow(new QLabel(tr("Version:")),
                 new QLabel(QString::fromStdString(info.getVersion())));
  layout->addRow(new QLabel(tr("Author:")),
                 new QLabel(QString::fromStdWString(info.getAuthor())));
  layout->addRow(new QLabel(tr("Year:")),
                 new QLabel(QString::fromStdWString(info.getYear())));

  QLabel *descriptionLabel =
      new QLabel(QString::fromStdWString(info.getDescription()));
    ...
    for (const AlgorithmParameterDescription &param : info.getParameters()) {
      ...
      if (param.getType() == AlgorithmParameterDescription::TYPE_DOUBLE) {
        ...
        if (param.displayLineEdit()) {
          ...
          le->setValidator(new QDoubleValidator(param.getDoubleMinValue(),
                                                param.getDoubleMaxValue(),
                                                param.getDoublePrecision()));
        ...
        if (param.getDoublePrecision() >= 0 && param.displaySlider()) {
          // add a slider aswell
          slider = new DoubleSlider(param.getDoubleMinValue(),
                                    param.getDoubleMaxValue(),
                                    param.getDoublePrecision());
        ...
        if (slider) {
          ...
          if (le) {
            QObject::connect(slider, SIGNAL(valueChanged(QString)), le,
                             SLOT(setText(QString)));
            QObject::connect(le, SIGNAL(textChanged(QString)), slider,
                             SLOT(setValue(QString)));
          }

          if (sb) {
            QObject::connect(slider, SIGNAL(valueChanged(double)), sb,
                             SLOT(setValue(double)));
            QObject::connect(sb, SIGNAL(valueChanged(double)), slider,
                             SLOT(setValue(double)));
          }
        }
      } else if (param.getType() == AlgorithmParameterDescription::TYPE_INT) {
        ...
        if (param.displayLineEdit()) {
          le = new QLineEdit();
          le->setReadOnly(param.readOnly());
          le->setValidator(new QIntValidator(param.getIntMinValue(),
                                             param.getIntMaxValue()));
          le->setText(QString::number(value));
          dataLayout->addWidget(le);
          dataWidget = le;
        }
        ...
        cb->setSizePolicy(QSizePolicy::MinimumExpanding,
                          QSizePolicy::Preferred);
        dataLayout->addWidget(cb);

        for (const auto &p : param.getStringList()) {
          cb->addItem(QString::fromStdWString(p.second),
                      QString::fromStdString(p.first));
        }
        int defaultIndex =
            cb->findData(QString::fromStdString(param.getStringDefaultValue()));
        if (description.hasStringParameter(param.getID())) {
          defaultIndex = cb->findData(QString::fromStdString(
              description.getStringParameter(param.getID())));
        }
      ...
      if (param.displaySetDefaultButton() && !param.readOnly()) {
        QPushButton *defaultButton =
            new DefaultButton(tr("Default"), param.getID(), dataWidget);
        defaultButton->setSizePolicy(QSizePolicy::Minimum,
                                     QSizePolicy::Preferred);
        defaultButton->setWhatsThis(
            tr("Reset the parameter to its default value"));
        defaultButton->setToolTip(defaultButton->whatsThis());
        dataLayout->addWidget(defaultButton);
        QObject::connect(defaultButton, SIGNAL(clicked(bool)), this,
                         SLOT(defaultButtonClicked()));
      }

      // create update timer
      if (param.updateIntervalInMS() >= 0) {
        new AlgorithmDialogParameterUpdateTimer(
            param, mCurrentAlgorithmParameters, dataWidget,
            scrollContentWidget);
      }

      paramsBoxLayout->addRow(label, dataLayout);

      mAlgorithmWidgetConnectionList.append(
          qMakePair(param.getID(), dataWidget));
    }
  }
...
void AlgorithmDialog::defaultButtonClicked() {
  ...
  const AlgorithmInformation::ParameterType &param =
      mCurrentAlgorithmInformation->getParameter(defaultButton->getID());

  switch (param.getType()) {
  case AlgorithmParameterDescription::TYPE_DOUBLE:
    if (param.displayLineEdit()) {
      dynamic_cast<QLineEdit *>(dataWidget)
          ->setText(QString::number(param.getDoubleDefaultValue()));
    }
    if (param.displaySpinBox()) {
      dynamic_cast<QDoubleSpinBox *>(dataWidget)
          ->setValue(param.getDoubleDefaultValue());
    }
    break;
  case AlgorithmParameterDescription::TYPE_INT:
    if (param.displayLineEdit()) {
      dynamic_cast<QLineEdit *>(dataWidget)
          ->setText(QString::number(param.getIntDefaultValue()));
    }
    if (param.displaySpinBox()) {
      dynamic_cast<QSpinBox *>(dataWidget)
          ->setValue(param.getIntDefaultValue());
    }
    break;
  ...
  case AlgorithmParameterDescription::TYPE_BOOL:
    dynamic_cast<QCheckBox *>(dataWidget)
        ->setChecked(param.getBoolDefaultValue());
    break;
  default:
    EPT_EXCEPT(EptException::ERR_NOT_IMPLEMENTED,
               "Default button does not implement type.");
  }
}
