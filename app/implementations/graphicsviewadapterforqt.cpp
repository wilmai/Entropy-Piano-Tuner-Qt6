...
GraphicsViewAdapterForQt::GraphicsViewAdapterForQt(QWidget *parent,
                                                   DrawerBase *drawer,
                                                   QRectF sceneRect)
    : QGraphicsView(parent), GraphicsViewAdapter(), mDrawer(drawer),
      mScene(sceneRect), mSceneRect(sceneRect) {
  setScene(&mScene);
...
GraphicsItem *GraphicsViewAdapterForQt::drawLine(double x1, double y1,
                                                 double x2, double y2,
                                                 PenType pen) {
  if (x1 < 0 || x1 > 1) {
    return nullptr;
  ...
  QGraphicsLineItem *line(
      mScene.addLine(convertRelToAbsLine(0, 0, x2 - x1, y2 - y1), getPen(pen)));
...
GraphicsItem *
GraphicsViewAdapterForQt::drawChart(const std::vector<Point> &points,
                                    PenType pen) {
  if (points.size() <= 1) {
    return nullptr;
  }
  ...
  for (; firstValidPointIndex < points.size(); firstValidPointIndex++) {
    if (points[firstValidPointIndex].x < 0 ||
        points[firstValidPointIndex].x > 1) {
      continue;
    }
    if (points[firstValidPointIndex].y < 0 ||
        points[firstValidPointIndex].y > 1) {
      continue;
    }
    path.moveTo(convertRelToAbs(QPointF(points[firstValidPointIndex].x,
                                        points[firstValidPointIndex].y)));
    break;
  }
...
GraphicsItem *GraphicsViewAdapterForQt::drawFilledRect(double x, double y,
                                                       double w, double h,
                                                       PenType pen,
                                                       FillTypes fill) {
  if (x < 0 || x > 1) {
  ...
  return new GraphicsItemForQt(
      this, mScene.addRect(QRectF(convertRelToAbs(QPointF(x, y)),
                                  convertRelToAbs(QSizeF(w, h))),
                           getPen(pen), getFill(fill)));
}
...
QSizeF GraphicsViewAdapterForQt::convertRelToAbs(const QSizeF &s) const {
  return QSizeF(s.width() * mSceneRect.width(),
                s.height() * mSceneRect.height());
}
...
QLineF GraphicsViewAdapterForQt::convertRelToAbsLine(qreal x1, qreal y1,
                                                     qreal x2, qreal y2) const {
  return QLineF(x1 * mSceneRect.width(), y1 * mSceneRect.height(),
                x2 * mSceneRect.width(), y2 * mSceneRect.height());
}
