...
const PianoFileIOXml::FileVersionType
    PianoFileIOXml::MIN_SUPPORTED_FILE_VERSION(1);
...
void PianoFileIOXml::write(QIODevice *device, const Piano &piano) const {
  ...
  writer.writeTextElement("manufactionLocation",
                          piano.getManufactionLocation());
  ...
  for (size_t keyIndex = 0; keyIndex < keyboard.size(); ++keyIndex) {
    ...
    writer.writeAttribute("measuredInharmonicity",
                          key.getMeasuredInharmonicity());
  ...
  const std::string &currentAlgorithm =
      CalculationManager::getSingleton().getCurrentAlgorithmId();
  const std::vector<SingleAlgorithmParametersPtr> &parameters =
      piano.getAlgorithmParameters().getParameters();
  ...
  if (writer.hasError()) {
    EPT_EXCEPT(EptException::ERR_CANNOT_WRITE_TO_FILE,
               "An error occured when writing to an xml stream.")
  }
  ...
  while (!reader.atEnd()) {
    ...
    if (reader.name() == FILE_TYPE_NAME.toStdString()) {
      ...
      if (mFileVersion == UNSET_FILE_VERSION) {
        LogW("No file version specified. Trying to continue with minimum "
             "supported.");
      ...
      if (mFileVersion < MIN_SUPPORTED_FILE_VERSION) {
        ...
        errormsg << "File version " << mFileVersion
                 << " is not supported anymore. Minimum file version is "
      ...
      while (!reader.atEnd()) {
        ...
        if (reader.name() == "name") {
          ...
          // read keyboard data
          int numberOfKeys = reader.queryIntAttribute(
              "numberOfKeys", keyboard.getNumberOfKeys());
          int keyNumberOfA = reader.queryIntAttribute(
              "keyNumberOfA", keyboard.getKeyNumberOfA4());
          int numberOfBassKeys = reader.queryIntAttribute(
              "numberOfBassKeys", keyboard.getNumberOfBassKeys());
          ...
          while (!reader.atEnd()) {
            ...
            // read data
            reader.queryRealAttributeRef("recordedFrequency",
                                         key.getRecordedFrequency());
            reader.queryRealAttributeRef("measuredInharmonicity",
                                         key.getMeasuredInharmonicity());
            reader.queryRealAttributeRef("computedFrequency",
                                         key.getComputedFrequency());
            reader.queryRealAttributeRef("tunedFrequency",
                                         key.getTunedFrequency());
            reader.queryBoolAttributeRef("recorded", key.isRecorded());
            reader.queryRealAttributeRef("quality",
                                         key.getRecognitionQuality());
          ...
          if (reader.hasAttribute("current")) {
            CalculationManager::getSingleton()
                .setCurrentAlgorithmInformationById(
                    reader.queryStringAttribute("current"));
          }
          ...
          while (!reader.atEnd()) {
            ...
            if (reader.name() == "algorithm") {
              ...
              SingleAlgorithmParametersPtr ad =
                  piano.getAlgorithmParameters().getOrCreate(algorithmName);
              ...
              while (!reader.atEnd()) {
                ...
                if (reader.isEndElement() && reader.name() == "algorithm") {
                  break;
                }
                ...
                if (reader.name() == "parameter") {
                  ...
                  if (type == "double") {
                    ...
                    LogW("Unknown parameter type '%s' with id '%s'",
                         type.c_str(), id.c_str());
                  }
