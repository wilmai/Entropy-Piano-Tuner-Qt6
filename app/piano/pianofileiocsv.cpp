...
void PianoFileIOCsv::write(QIODevice *device, const Piano &piano) const {
  ...
  // only write
  // key index + 1, inharmonicity, recorded frequency, computedAA_AttributeCount
  // frequency, tuned frequency, quality

  // header
  stream << "\"Key index\",\"Inharmonicity\",\"Recorded frequency\",\"Recorded "
            "deviation\",\"Computed frequency\",\"Computed deviation\",\"Tuned "
            "frequency\",\"Tuned deviation\",\"Quality\""
         << Qt::endl;
  ...
  auto cents = [A4](int i, double f, double fA4) {
    ...
    if (f == 0 or fA4 == 0)
      ...
      return 1200.0 * std::log(f / fA4 / ET) / std::log(2.0);
  };
  for (int i = 0; i < piano.getKeyboard().getNumberOfKeys(); ++i) {
    ...
    stream << QString("%1,").arg(i + 1)
           ...
           << QString("%1,").arg(cents(i, key.getRecordedFrequency(),
                                       Akey.getRecordedFrequency()),
                                 6, 'g', 5)
           << QString("%1,").arg(key.getComputedFrequency(), 8, 'g', 5)
           << QString("%1,").arg(cents(i, key.getComputedFrequency(),
                                       Akey.getComputedFrequency()),
                                 6, 'g', 5)
           << QString("%1,").arg(key.getTunedFrequency(), 8, 'g', 5)
           << QString("%1,").arg(
                  cents(i, key.getTunedFrequency(), piano.getConcertPitch()), 6,
                  'g', 5)
  ...
  if (stream.status() == QTextStream::WriteFailed) {
    EPT_EXCEPT(EptException::ERR_CANNOT_WRITE_TO_FILE,
               "Could not write to QTextStream when writing a csv file.");
  }
}
